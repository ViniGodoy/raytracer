...
public class Compounds {
    ...
    public static Compound newCylinder(float bottom, float top, float radius, float bevelRadius, Material mtrl) {
        ...
        if (bevelRadius > 0) {
            Torus torus = new Torus(radius - bevelRadius, bevelRadius, mtrl);
    ...
    public static Compound newRoundBox(boolean faces, Vector3 minCorner, Vector3 maxCorner, float bevelRadius, Material mtrl) {
        ...
        OpenCylinder topFrontBack = new OpenCylinder(
        ...
        OpenCylinder topLeftRight = new OpenCylinder(
        ...
        OpenCylinder bottomFrontBack = new OpenCylinder(
                -(maxCorner.getX() - minCorner.getX() - 2.0f * bevelRadius) / 2.0f,
                (maxCorner.getX() - minCorner.getX() - 2.0f * bevelRadius) / 2.0f,
                bevelRadius, mtrl);
        ...
        OpenCylinder bottomLeftRight = new OpenCylinder(
                -(maxCorner.getZ() - minCorner.getZ() - 2.0f * bevelRadius) / 2.0f,
                (maxCorner.getZ() - minCorner.getZ() - 2.0f * bevelRadius) / 2.0f,
                bevelRadius, mtrl);
        ...
        OpenCylinder verticalRightFront = new OpenCylinder(minCorner.getY() + bevelRadius, maxCorner.getY() - bevelRadius, bevelRadius, mtrl);
        ...
        OpenCylinder verticalLeftBack = new OpenCylinder(minCorner.getY() + bevelRadius, maxCorner.getY() - bevelRadius, bevelRadius, mtrl);
    ...
    public static Compound newBowl(boolean round, float innerRadius, float outerRadius, float openAngle, Material mtrl) {
        ...
        float avgRadius = (innerRadius + outerRadius) / 2.0f;

        if (round) {
            float a = avgRadius * (float) Math.sin(openAngle);
            float b = (outerRadius - innerRadius) / 2.0f;
            c.addInstance(new Torus(a, b, mtrl))
                    .translate(0, avgRadius * (float) Math.cos(openAngle), 0);
        } else {
            float inner = (float) (innerRadius * Math.sin(openAngle));
            float outer = (float) (outerRadius * Math.sin(openAngle));
